...
export const getTokenFromUrl = () => {
    const hash = window.location.hash.substring(1); // '#'の後ろの部分を取得
    const tokenInfo = hash.split('&').reduce((initial, item) => {
        let parts = item.split('='); // 各部分を'='で分割
        initial[parts[0]] = decodeURIComponent(parts[1]); // キーと値をオブジェクトに格納
        return initial;
    }, {});
    return tokenInfo.access_token; // アクセストークンを返す
};

export const accessUrl = `${authEndpoint}?client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scopes.join(' ')}&response_type=token&show_dialog=true`;
